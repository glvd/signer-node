...
type Contracter interface {
	...
	Video() (*video.Video, *bind.TransactOpts, *ethclient.Client)
}
...
func (c contract) Video() (*video.Video, *bind.TransactOpts, *ethclient.Client) {
	// TODO
	auth, err := bind.NewTransactor(strings.NewReader(c.keystore), "123")
	if err != nil {
		log.Fatal(err)
	...
	address := common.HexToAddress(videoAddr)
	instance, err := video.NewVideo(address, client)
	if err != nil {
		log.Fatal(err)
	}
